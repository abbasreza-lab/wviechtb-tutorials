...
# in RStudio because the pane for plots at the bottom right is a bit small;
# if this happen, you just have to make the size of the plotting pane larger
...
# (https://en.wikipedia.org/wiki/Hexadecimal); e.g., these two are the same
...
# can also directly specify colors in terms of RGB intensities, but note that
# (by default) the intensities need to be given on a 0-1 scale
...
# note: if you get "Error in plot.window(...): need finite 'xlim' values",
# then your dataset does not include the 'pss' (and probably also not the
# 'posaff') variable we created earlier (in rode04)
...
# occurred in the dataset; can use this to plot x and y values from 'res'
