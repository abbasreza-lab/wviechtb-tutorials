...
# Added from chat:
z <- order(TukeyHSD(res)$marital[,"p adj"])
TukeyHSD(res)$marital[z,]
...
#first one is wrong code
cor(dat$posaff, dat$negaff, dat$pss)

#has missing values therefore returns NA

cor(dat[c("posaff", "negaff", "pss")])

#corrected with 'use' argument (na.rm argument does not work for cov function)
...
# smoothers (non-paramtric ways of examining realtionship between predictor and outcome)
